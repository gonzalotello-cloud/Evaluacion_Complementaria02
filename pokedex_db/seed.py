"""Módulo de siembra (seeding) de datos iniciales para la base de datos Pokedex.

Este archivo puebla la base de datos con registros coherentes y suficientes
para que todas las consultas generen resultados válidos, garantizando la
idempotencia del script mediante una limpieza previa coordinada de las tablas.
"""
...
def poblar_datos(session: Session) -> None:
    """Inserta los datos mínimos requeridos y coherentes en la base de datos."""
    # --------------------------------------------------------------------------
    # 1. REGIONES (Mínimo 4 de generaciones distintas)
    # --------------------------------------------------------------------------
    ...
    session.commit()  # Guardamos para obtener los IDs generados automáticamente

    # --------------------------------------------------------------------------
    # 2. TIPOS ELEMENTALES (Mínimo 8, incluyendo Fuego, Agua y Eléctrico)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # 3. ENTRENADORES (Mínimo 8 distribuidos, 2 campeones, 2 en la misma región)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # 4. POKÉMON (Mínimo 20 distribuidos, 3 shiny, 2 con apodo, nivel >80 y <20)
    # --------------------------------------------------------------------------
    pokemons = [
        # Pokémon de Ash
        ...
        Pokemon(nombre="Arcanine", nivel=68, puntos_vida=240, es_shiny=False, apodo="Perron", entrenador_id=gary.id), # Apodo 2
        Pokemon(nombre="Pidgeot", nivel=62, puntos_vida=210, es_shiny=False, apodo="Pajaron", entrenador_id=gary.id),
        # Pokémon de Ethan
        Pokemon(nombre="Typhlosion", nivel=82, puntos_vida=270, es_shiny=False, apodo=None, entrenador_id=ethan.id), # Nivel >80
        Pokemon(nombre="Gyarados", nivel=45, puntos_vida=310, es_shiny=True, apodo=None, entrenador_id=ethan.id), # Shiny 2
        ...
        Pokemon(nombre="Piplup", nivel=5, puntos_vida=20, es_shiny=True, apodo=None, entrenador_id=brendan.id),
        # Pokémon de May
        Pokemon(nombre="Blaziken", nivel=72, puntos_vida=245, es_shiny=False, apodo=None, entrenador_id=may.id),
        Pokemon(nombre="Beautifly", nivel=38, puntos_vida=150, es_shiny=True, apodo=None, entrenador_id=may.id),    # Shiny 3
        # Pokémon de Lucas
    ...
    # --------------------------------------------------------------------------
    # 5. ASIGNACIÓN DE TIPOS (Mínimo 6 Pokémon con doble tipo)
    # --------------------------------------------------------------------------
    asignacion_tipos = [
    ...
    # --------------------------------------------------------------------------
    # 6. BATALLAS Y PARTICIPACIONES (Mínimo 6 batallas, al menos 1 empate)
    # --------------------------------------------------------------------------
    ...
    # Batalla 2: Victoria de Ethan sobre Lyra
